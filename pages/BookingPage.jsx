...
const BookingPage = () => {
  ...
  const submitDriverForm = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setShowDriverForm(false); // close driver modal
...
<div className="feedback-section" style={{ marginTop: "16px" }}>
  <h3 style={{ color: "#1e3d59", marginBottom: "8px" }}>User Feedback</h3>
  {feedbackList.length === 0 ? (
    <p style={{ fontSize: "13px", color: "#555" }}>No feedback yet.</p>
  ) : (
    feedbackList.map((fb) => (
      <div
        key={fb.id}
      ...
      >
        {/* Star rating */}
        <p style={{ margin: "0 0 4px 0", fontWeight: "600", color: "#ffb400" }}>
  ...
  const bookingForPDF = {
    id: data.bookingId || Math.floor(100000 + Math.random() * 900000),
  ...
  fetch("http://localhost:5000/api/bookings/confirm", {
    ...
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ ...bookingForPDF, vehicleId: vehicle.id }),
