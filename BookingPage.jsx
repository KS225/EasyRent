...
const BookingPage = () => {
  ...
  const fetchORSdistance = async () => {
    ...
    try {
      ...
      const distance = data?.features?.[0]?.properties?.segments?.[0]?.distance / 1000; // meters to km
      if (!distance) return alert("Failed to fetch distance from ORS");
      setDistanceKm(distance.toFixed(2));
      return distance;
    } catch (err) {
      console.error("ORS Error:", err);
      alert("Error fetching route distance");
    }
  };
  ...
  const calculatePrice = async () => {
    ...
    const distance = await fetchORSdistance();
    if (!distance) return;

    const days = calculateDays();
    const total = distance * 15 + vehicle.price_per_day * days; // ₹15 per km + vehicle daily price
    ...
    <div className="booking-container">
      ...
      <div className="trip-details">
        ...
        <select value={pickup} onChange={(e) => setPickup(e.target.value)}>
          ...
          {Object.keys(locationCoords).map(city => <option key={city} value={city}>{city}</option>)}
        </select>
