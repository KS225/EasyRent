...
app.post("/api/bookings", async (req, res) => {
  ...
  const { pickup, drop, dateFrom, dateTo, price, driverName, driverContact, driverAge, driverLicense } = req.body;

  if (!pickup || !drop || !dateFrom || !dateTo || !price || !driverName || !driverContact || !driverAge || !driverLicense) {
    return res.status(400).json({ message: "All fields are required" });
  }
  ...
  try {
    await db.execute(
      `INSERT INTO bookings 
       (user_id, pickup_location, drop_location, date_from, date_to, price, driver_name, driver_contact, driver_age, driver_license)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [req.session.userId, pickup, drop, dateFrom, dateTo, price, driverName, driverContact, driverAge, driverLicense]
    );
